...
class _ProductDetailState extends State<ProductDetail>
  ...
  void initState() {
    ...
    _textAnimationController =
        AnimationController(vsync: this, duration: Duration(seconds: 0));
    printLog(json.encode(widget.product), name: "WIDGET PRODUCT");
    loadDetail();
    // loadLikeProduct();
  }
    ...
    if (widget.product != null) {
      if (Session.data.getBool('isLogin')!) {
        await productProvider
            .hitViewProducts(widget.product!.id)
            .then((value) async => await productProvider.fetchRecentProducts());
      }
      setState(() {
    ...
    if (widget.product == null) {
      if (widget.slug == null) {
        await Provider.of<ProductProvider>(context, listen: false)
            .fetchProductDetail(widget.productId, videoId: widget.videoId)
            .then((value) async {
          setState(() {
          ...
          if (productModel!.images!.isNotEmpty ||
            ...
            itemSlider = [
              ...
              for (var i = 1; i < productModel!.images!.length; i++)
                imagecache(i, productModel!.images![i].src!),
          ...
          if (widget.product!.images!.isNotEmpty ||
            ...
            itemSlider = [
              ...
              for (var i = 1; i < widget.product!.images!.length; i++)
                imagecache(i, widget.product!.images![i].src),
      ...
      child: Column(
        ...
        children: [
          ...
          HtmlWidget(
            ...
            onTapUrl: (p0) => Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) =>
                      WebViewScreen(fromNotif: false, title: "", url: p0),
                )).then((value) {
              return true;
            }),
            textStyle: TextStyle(
      ...
      actions: [
        ...
        InkWell(
          onTap: () {
            shareLinks(
                'product',
                "${model.link}${Provider.of<UserProvider>(context, listen: false).refModel != null ? Provider.of<UserProvider>(context, listen: false).refModel!.referralPath : ''}",
                context,
                locale);
          },
          child: Container(
