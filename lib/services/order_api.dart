...
class OrderAPI {
  checkoutOrder(order, String currency) async {
    var response = await baseAPI.getAsync(
        '$orderApi?order=$order&currency=$currency',
        isOrder: true,
        printedLog: true);
    return response;
  }
  ...
  loadProductCart(String? include) async {
    ...
    printLog(data.toString());
    var response = await baseAPI.postAsync(
      '$customProductUrl' + "?lang=${Session.data.getString('language_code') ?? ""}",
      data,
      isCustom: true,
    );
    return response;
  ...
  checkoutData(
    ...
    var response = await baseAPI.postAsync(
        '$checkoutDatas?currency=${Session.data.getString('currency_code')}',
        data,
        isCustom: true);
    return response;
  }
  ...
  placeOrder(
    ...
    Map data = {
      ...
      if (Session.data.containsKey('ref_path'))
        "referral": Session.data.getString('ref_path')
    };
    printLog("data place : ${json.encode(data)}");
    var response = await baseAPI.postAsync(
        '$placeOrders?currency=${Session.data.getString('currency_code')}',
        data,
        isCustom: true);
