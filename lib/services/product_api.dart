...
class ProductAPI {
  newSearchProduct(String? product) async {
    var response = await baseAPI.getAsync(
        'search?product=$product&lang=${Session.data.getString('language_code') ?? ""}',
        isCustom: true,
        printedLog: true);
    return response;
  }
  ...
  fetchDetailProduct(String? productId, {String? videoId = ""}) async {
    Map data = {
      "id": productId,
      // "lang": Session.data.getString("language_code"),
      "cookie": Session.data.containsKey("cookie")
