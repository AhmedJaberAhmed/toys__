...
class BaseWooAPI {
  ...
  Future<dynamic> getAsync(String endPoint,
    ...
    if (isOrder || isWebView) {
      return url;
    }
    Map<String, String> headers = {};
    if (Session.data.containsKey("home-revo-etag") && isHomeAPI) {
      headers['If-None-Match'] = Session.data.getString('home-revo-etag')!;
    }
  ...
  getOAuthURL(String requestMethod, String endpoint, version, bool isCustom,
      bool isOrder) {
    var consumerKey = this.consumerKey;
    var consumerSecret = this.consumerSecret;
  ...
  Future<dynamic> postAsync(
    ...
    if (!isReview) {
      ...
      request.body = data.isEmpty ? '' : json.encode(data);
      var response =
          await client.send(request).then((res) => res.stream.bytesToString());
      dataResponse = await json.decode(response);
    }
    if (isReview) {
      try {
        Map<String, String> headers = {
        ...
        if (data['media[]'] != null) {
          data['media[]'].forEach((element) async {
        ...
        if (streamedResponse.statusCode == 200) print("uploaded");
        var response = http.Response.fromStream(streamedResponse);
        await response.then((value) {
          printLog(value.body, name: "Value body");
          dataResponse = json.decode(value.body);
        });
      } catch (e) {
        printLog(e.toString());
        rethrow;
