...
class LoginProvider with ChangeNotifier {
  ...
  setBiometric(val) {
    isBiometric = val;
    printLog("${Session.data.getString("usernameBio")} username bio set bio");
    printLog("${Session.data.getString("passwordTemp")} pw bio set bio");
    if (Session.data.containsKey("usernameBio") &&
        Session.data.containsKey("passwordTemp")) {
      printLog("masuk if set bio");
    ...
    try {
      ...
      await LoginAPI().loginByDefault(username, password).then((data) {
        ...
        if (result['cookie'] != null) {
          ...
          Session().saveUser(user, result['cookie'], username, password);
  ...
  Future<void> signInOTPv2(
    ...
    await LoginAPI()
      ...
      if (data.statusCode == 200) {
        ...
        Session.data.setString("usernameBio", responseJson['user_login']);
        Session.data.setString("passwordTemp", responseJson['token']);
