...
class CheckoutProvider with ChangeNotifier {
  ...
  setCourier(index, context) {
    ...
    for (int i = 0; i < shippingLines.length; i++) {
      if (shippingLines[i].methodTitle == "other_courier" ||
          shippingLines[i].methodId == "local_pickup_plus" ||
          shippingLines[i].methodId == "biteship") {
        for (int j = 0; j < shippingLines[i].couriers!.length; j++) {
          if (shippingLines[i].couriers![j].methodTitle == titleCourier) {
            tempShip = new ShippingLine(
                methodId:
                    "${shippingLines[i].methodId}:${shippingLines[i].couriers![j].courier}:${shippingLines[i].couriers![j].service}",
  ...
  setShipping(index, context) {
    if (shippingLines[index].methodId == "biteship") {
      titleCourier =
          "${AppLocalizations.of(context)!.translate('select_courier')!}";
    } else if (shippingLines[index].methodId == "local_pickup_plus") {
      titleCourier =
    ...
    indexShipping = index;
    grandTotal = total;
    newTotal = total! + shippingLines[index].totalCost!;
    shipping = "${shippingLines[index].methodTitle}";
    shippingCost =
        "${stringToCurrency(shippingLines[index].totalCost!, context)}";
    titleShipping = shippingLines[index].methodTitle == "other_courier"
        ? AppLocalizations.of(context)!.translate('other_courier')
        : shippingLines[index].methodId == "biteship"
            ? shippingLines[index].methodTitle
            : shippingLines[index].methodId == "local_pickup_plus"
                ? shippingLines[index].methodTitle
                : "${shippingLines[index].methodTitle} (${stringToCurrency(shippingLines[index].totalCost!, context)})";
  ...
  Future getCheckoutData(
      ...
      String? city,
      String? subdistrict,
      LocationCoordinate? location}) async {
    loading = true;
    notifyListeners();
    try {
