...
class DeeplinkConfig {
  ...
  Future<Widget> initUniLinks(BuildContext context) async {
    ...
    try {
      String? initialLink = await AppLinks().getInitialAppLink().then((value) {
        printLog(value.toString(), name: "init link");
    ...
    if (uri.pathSegments.isEmpty) {
      ...
      if (uri.pathSegments[1].isNotEmpty) {
        ...
        if (uri.queryParameters.isNotEmpty) {
          printLog("${uri.queryParameters['ref']}");
          Session.data.setString("ref_path", "${uri.queryParameters['ref']}");
        }
        if (fromNotif == true) {
          print("Detail Product");
          if (fromLaunchApp) {
      ...
      if (uri.pathSegments[1].isNotEmpty &&
          uri.pathSegments[1].toString() != "video") {
        print("Detail Blog");
      ...
      } else if (uri.pathSegments[1].toString() == "video") {
        debugPrint(uri.toString());
        debugPrint(uri.pathSegments[0]);
        debugPrint(uri.pathSegments[1]);
        if (fromLaunchApp) {
      ...
      if (fromNotif == true) {
        ...
        homeProvider.toggleNeedLoadingExternalLink(true);
        await Navigator.of(GlobalVariable.navState.currentContext!).push(
          MaterialPageRoute(
            builder: (context) => HomeScreen(),
          ),
        );
        homeProvider.toggleNeedLoadingExternalLink(false);
        await Navigator.of(GlobalVariable.navState.currentContext!)
            .push(MaterialPageRoute(
                builder: (context) => WebViewScreen(
                      title: "Push Notification",
